...
class PinoIrc {
  constructor (api) {
    ...
    this._ircClient.on('action', this.sendActionToTelegram.bind(this))
  ...
  initClient() {
    this._ircClient =  new irc.Client(config.pinoIrc.server, 'PinoOneeChan', {
     channels: [...this._groupsChannel.chatIds.keys(), ...config.pinoSup.ircChan],
     stripColors: true,
     userName: 'pino',
  ...
  sendMessageToTelegram (chatIds, to, from, message) {
    for (let chatId of chatIds) {
      this._api.sendMessage(
        chatId,
        `[IRC${to}] <b>${from}:</b> ${message}`,
        {parse_mode: 'HTML'}
      )
    }
  }

  sendActionToTelegram (from, to, text) {
    for (let chatId of this._groupsChannel.chatIds.get(to)) {
      this._api.sendMessage(
        chatId,
        `[IRC${to}] <i>${from} ${text}</i>`,
        {parse_mode: 'HTML'}
      )
    }
  }
